...
- `docs/llms.md` is the **AI-agent / LLM reference** — a single, machine-first page mirroring the public API surface, capabilities, usage recipes, and the disposal/thread-safety/lifecycle rules, so an agent can write correct MPath code without reading the whole docs tree. It is **canonical and must be kept in sync**: any change to a public type, method, constructor, setting, default, cost formula, or lifecycle rule must update `docs/llms.md` in the **same change** (its "Maintenance contract" section spells out exactly what to touch). Keep signatures copy-paste-correct — agents emit them verbatim.
